...
if(!["756890634790633573"].some(role => message.member.roles.cache.get(role)) && (!message.member.hasPermission("ADMINISTRATOR"))) 
return message.channel.send(new MessageEmbed().setDescription(` Dostum Yetkilerine Bir Bak Derim.Maalesef Yetkin Yetersiz`).setColor('0x800d0d').setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
  
const mutelog = message.guild.channels.cache.find(c => c.id === '779759458674540585')//Mute logu
const muterol = message.guild.roles.cache.find(r => r.id === '805520154455703613')//Muteli rolü
